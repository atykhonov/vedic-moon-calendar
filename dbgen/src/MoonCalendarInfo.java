...
public class MoonCalendarInfo {
	...
	public Rashi getMoonRashi() {
		return Rashi.getByNumber(rashiOfGraha[CH]);
	}

	public Rashi getSunRashi() {
		return Rashi.getByNumber(rashiOfGraha[SY]);
	}

	public Nakshatra getMoonNakshatra() {
		return Nakshatra.getByPosition(pos[CH]);
	}
	...
	private static String doubleToHour(double angle) {
		...
		System.out.println("Ostacha: " + ost);
	...
	public Date calcSunEclipse() {
		return calcEclipse(0);
	}

	public Date calcMoonEclipse() {
		return calcEclipse(1);
	}

	private Date calcEclipse(int planet) {

		int year = dateTime.getYear(); // getCalendar().get(Calendar.YEAR);
		int month = dateTime.getMonthOfYear(); // getCalendar().get(Calendar.MONTH)
												// + 1;
		int day = dateTime.getDayOfMonth(); // getCalendar().get(Calendar.DAY_OF_MONTH);
		int hour = dateTime.getHourOfDay(); // getCalendar().get(Calendar.HOUR_OF_DAY);
		int minutes = dateTime.getMinuteOfHour(); // getCalendar().get(Calendar.MINUTE);
		int seconds = dateTime.getSecondOfMinute(); // getCalendar().get(Calendar.SECOND);
		...
		if (planet == 0) {
			eclflag = sw.swe_sol_eclipse_when_glob(jdET, SweConst.SEFLG_SWIEPH,
					0, tret, 0, serr);
		} else if (planet == 1) {
			eclflag = sw.swe_lun_eclipse_when(jdET, SweConst.SEFLG_SWIEPH, 0,
					tret, 0, serr);
		} else {
		 ...
		 * calendar.set(Calendar.HOUR_OF_DAY, date.get)
		 */

		return SweDate.getDate(date.getJulDay());
	}
	...
	public void calc() {
		...
		for (int i = 1; i < 9; i++) {
			...
			ret = sw.swe_calc_ut(jdET, pl[i], flgs, dres, serr);
			if (ret < 0) {
	 ...
	 * 
	 * Calendar originCalendar = getCalendar(); Calendar modifiedCalendar =
	 * (Calendar) originCalendar.clone();
	 * 
	 * String originNakshatra = getMoonNakshatra().toString();
	 * 
	 ...
	 * calc();
	 * 
	 * String nakshatra = getMoonNakshatra().toString();
	 * 
	 * if (originNakshatra != nakshatra) { Calendar changedNakshatra =
	 * (Calendar) modifiedCalendar.clone(); result.add(changedNakshatra);
	 * 
